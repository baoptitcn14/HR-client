...
export class CreateCvComponent implements OnInit {
  ...
  selectSidebarItem(item: any) {
    ...
    if (!this.sidebarContent.show)
      this.sidebarContent.show = true;
  ...
  dropIntoCol(event: CdkDragDrop<any[]>, col?: IThanhPhan) {
    ...
    if (typeCode) {
      ...
      const listDataOfTypeCode = this.dataCvTemplate.filter(e => e.typeCode == typeCode).map(e => e.value);
      ...
      if (event.previousContainer === event.container) {
        ...
        if (col) {
          col._listChild = [...col._listChild!, ...listDataOfTypeCode.reduce((acc, e) => [...acc, ...e], [])];
        }
  ...
  private getTemplate() {
    this.http.get<IThanhPhan[]>('/assets/template.json').subscribe((data) => {
      ...
      const templateData = data.map((item: IThanhPhan) => ({ ...item, _css: JSON.parse(item.css ?? '{}') })) as IThanhPhan[];

      this.template = templateData.find((item: IThanhPhan) => item.inputType == 'template');
      this.listRow = templateData.filter((item: IThanhPhan) => item.inputType == 'row')
        .map(r => ({
          ...r, _listChild: templateData.filter(e => e.rowId == r.id).map(e => ({ ...e, _listChild: [] as IThanhPhan[], _listTypeCode: [] as string[] })),
          _panelSizes: templateData.filter(e => e.rowId == r.id).map(e => e._css['width']),
          _panelMinSizes: templateData.filter(e => e.rowId == r.id).map(e => 20)
        })) as IThanhPhan[];


      this.listCdkDropListConnectedTo = templateData.filter((item: IThanhPhan) => item.inputType == 'col').map(r => r.id) as string[];
    });
  }
  ...
  private getDataCvTemplate() {
    this.http.get<any[]>('/assets/data-cv-template.json').subscribe((data) => {
      ...
      this.listGroupDistinct = data.filter((item, i, arr) => arr.findIndex(e => e.typeCode == item.typeCode) == i)
